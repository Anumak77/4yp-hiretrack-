...
function CVEditMock() {
  ...
  const handleSaveCV = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        setModal({ show: true, title: 'Success', message: 'CV saved successfully!' });
      } else {
        setModal({ show: true, title: 'Error', message: 'Failed to save CV.' });
      }
    } catch (error) {
      console.error(error);
      setModal({ show: true, title: 'Error', message: 'Something went wrong while saving your CV!' });
    }
  };
