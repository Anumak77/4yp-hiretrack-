describe('Post Job Form', () => {
  ...
  it('allows a user to fill and submit the job posting form', () => {
    cy.get('input[name="Title"]').type('Senior Developers');
    ...
    cy.get('textarea[name="jobpost"]').type('Apply');
    cy.get('input[name="OpeningDate"]').type('2023-01-01');
    cy.get('input[name="Deadline"]').type('2023-02-01');
    cy.get('input[name="StartDate"]').type('2023-03-01');
    cy.get('input[name="date"]').type('2023-03-01');
