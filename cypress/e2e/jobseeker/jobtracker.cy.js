describe("Job Tracker Jobseeker", () => {
    beforeEach(() => {
      cy.visit("/jobtracker"); // Change path based on your router
    });
    ...
    it("shows job options dropdown and allows selection", () => {
      cy.get(".jobtracker-select").should("exist").select("Software Engineer");
      cy.get(".jobtracker-select").should("have.value", "Software Engineer");
    });
    ...
    it("adds a job application when fields are filled", () => {
      cy.get(".jobtracker-select").select("Data Analyst");
